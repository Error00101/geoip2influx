            ...
            if not line:
                ...
                if ip_type in monitored_ip_types and ip:
                    ...
                    if info:
                        ...
                        geohash_tags['measurement'] = geo_measurement
                        geohash_fields['latitude'] = info.location.latitude if info.location.latitude else "-"
                        geohash_fields['longitude'] = info.location.longitude if info.location.longitude else "-"
                        geohash_fields['ip'] = ip
                        geohash_fields['geohash'] = geohash
                        ...
                        with Sender(influxdb_host, influxdb_port) as sender:
                            sender.row(influxdb_database+'_geo', symbols=geohash_tags, columns=geohash_fields)
                            sender.flush()
                else:
                ...
                if send_logs:
                    ...
                    if ip_type in monitored_ip_types and ip:
                        ...
                        if info:
                            ...
                            log_data_tags['measurement'] = log_measurement
                            ...
                            with Sender(influxdb_host, influxdb_port) as sender:
                                sender.row(influxdb_database+'_log', symbols=log_data_tags, columns=log_data_fields)
                                sender.flush()
